...
function StatusChip({ status }: { status: string | null }) {
  const cls =
    status === "overdue" ? "chip-red" :
    status === "graded" ? "chip-green" :
    status === "submitted" ? "chip-blue" :
    "chip-amber";
...
function AssignmentRow({ a }: { a: Assignment }) {
  return (
    <tr className="border-t border-gray-100 hover:bg-gray-50">
      <td className="py-2 px-3 whitespace-nowrap text-gray-600 text-sm">{a.subject}</td>
      <td className="py-2 px-3">{a.title}</td>
      <td className="py-2 px-3 text-gray-500 text-sm whitespace-nowrap">{a.normalized?.type}</td>
      <td className="py-2 px-3 whitespace-nowrap text-sm">{a.due_or_date}</td>
      <td className="py-2 px-3"><StatusChip status={a.status} /></td>
    </tr>
  );
}

function AssignmentTable({ title, rows, accent }: { title: string; rows: Assignment[]; accent: string }) {
  ...
  return (
    <div className="mt-4">
      ...
      <table className="w-full text-sm mt-1">
        ...
        <tbody>
          {rows.map((a) => <AssignmentRow key={a.id} a={a} />)}
        </tbody>
      </table>
    </div>
  );
...
function GradeTrendBlock({ childId }: { childId: number }) {
  const { data } = useQuery({
    queryKey: ["grade-trends", childId],
    queryFn: () => api.gradeTrends(childId),
  });
...
export default function Today() {
  ...
  const sync = async () => { await api.syncNow(); setTimeout(() => qc.invalidateQueries({ queryKey: ["today"] }), 500); };
    ...
    <div>
      ...
      {data.children.map((kid) => (
        <section key={kid.child.id} className="mb-10 bg-white border border-gray-200 rounded p-5 shadow-sm">
          <h3 className="text-lg font-bold border-b border-gray-100 pb-2 mb-2">
            {kid.child.display_name} <span className="text-gray-500">· {kid.child.class_section}</span>
          </h3>
          ...
          <GradeTrendBlock childId={kid.child.id} />
        </section>
      ))}
